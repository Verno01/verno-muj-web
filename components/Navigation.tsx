    ...
    <header className="verno-nav-shell">
      ...
      <div id="verno-mobile-menu" className={`verno-mobile-menu ${open ? 'is-open' : ''}`}>
        <div className="verno-mobile-menu-inner">
          ...
          <a href="tel:+420705911941" className="verno-mobile-call" onClick={close}>Zavolat</a>
        </div>
      </div>
      ...
      <style>{`
        ...
        .verno-mobile-trigger{display:none}
        .verno-mobile-menu{display:none}

        @media (max-width:760px){
          .verno-nav-inner{width:min(100% - 30px,1040px);height:58px;grid-template-columns:1fr auto;gap:18px}
          ...
          .verno-mobile-menu.is-open{max-height:340px;border-top-width:1px}
          .verno-mobile-menu-inner{width:min(100% - 30px,1040px);margin:0 auto;padding:8px 0 16px;display:flex;flex-direction:column}
          ...
          .verno-mobile-menu-inner .verno-mobile-call{margin-top:12px;min-height:42px;width:max-content;padding:0 18px;border:0;background:#efff63;font-weight:700}
        }
      `}</style>
    </header>
  )
